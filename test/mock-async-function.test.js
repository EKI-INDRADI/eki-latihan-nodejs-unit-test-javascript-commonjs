...
test("mock async function", async () => {
  ...
  await expect((0, _async.getBalance)("eki", from)).resolves.toEqual({
    name: "eki",
    balance: 1000
  });
...
test.failing("mock async function rejected", async () => {
  ...
  await (0, _async.getBalance)("eki", from);
});
test("mock async function error matchers", async () => {
  const from = jest.fn();
  from.mockRejectedValueOnce("Rejected");
  await expect((0, _async.getBalance)("eki", from)).rejects.toBe("Rejected");
});